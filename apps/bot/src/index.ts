...
const optionMap: Record<string, Array<ReturnType<typeof string> | ReturnType<typeof integer> | ReturnType<typeof user>>> = {
  ...
  'ingame-record': [string('case-id', 'Record case ID', true)],
  'ingame-records': [string('player', 'Player identifier')],
  ...
  'discord-record': [string('case-id', 'Record case ID', true)],
  'discord-records': [user('user', 'Discord member')],
  command: [string('name', 'Command name', true)],
  help: [string('command', 'Command name')],
...
async function heartbeat(): Promise<void> {
  ...
  await Promise.all(Array.from(client.guilds.cache.values()).map(async (guild) => {
    await prisma.botHeartbeat.upsert({
      ...
      create: { guildId: guild.id, status: 'online', latencyMs: Math.max(0, client.ws.ping), uptimeSeconds: Math.floor((Date.now() - bootedAt) / 1000), version, lastSeen: now, maintenance: false },
    });
  }));
}
...
async function controlLoop(): Promise<void> {
  ...
  const request = await prisma.controlRequest.findFirst({ where: { status: 'pending', type: { in: ['RESTART', 'MAINTENANCE_ON', 'MAINTENANCE_OFF'] } }, orderBy: { createdAt: 'asc' } });
  if (!request) return;
...
async function handleCommand(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (remaining > 0) { await interaction.reply({ content: `Please wait ${Math.ceil(remaining / 1000)}s before using this command again.`, ephemeral: true }); return; }
  ...
  switch (interaction.commandName) {
    case 'session-start': {
      if (session) { await interaction.reply({ content: `A roleplay session is already active: **${session.serverName || config.serverName}**.`, ephemeral: true }); return; }
      ...
      await interaction.reply({ content: `✅ Roleplay session started: **${serverName}**.`, ephemeral: true }); return;
    }
    case 'session-end': {
      if (!session) { await interaction.reply({ content: 'No active roleplay session.', ephemeral: true }); return; }
      ...
      await writeAudit(interaction.guildId, interaction.user.id, 'SESSION_ENDED', session.id); await interaction.reply({ content: `🏁 Session ended. Peak players: **${session.peakPlayers}**.`, ephemeral: true }); return;
    }
    case 'session-info':
    case 'session-roster':
      await interaction.reply({ content: session ? `🎮 **${session.serverName || config.serverName}**\nPlayers: **${session.currentPlayers}/${session.capacity}**\nPeak: **${session.peakPlayers}**\nStarted: <t:${Math.floor(session.startedAt.getTime() / 1000)}:R>` : '⚫ No active roleplay session.', ephemeral: true }); return;
    case 'server-startup': {
      ...
      await interaction.reply({ content: '🟢 Startup announcement sent.', ephemeral: true }); return;
    }
    case 'server-close':
      await sendAutomation(interaction.guildId, 'sessionEnd', { serverName: session?.serverName || config.serverName, players: session?.currentPlayers || 0, capacity: session?.capacity || config.serverCapacity }); await interaction.reply({ content: '🏁 Server close announcement sent.', ephemeral: true }); return;
    case 'server-full': {
      const players = interaction.options.getInteger('players') ?? session?.currentPlayers ?? session?.capacity ?? config.serverCapacity;
      await sendAutomation(interaction.guildId, 'serverFull', { players, capacity: session?.capacity || config.serverCapacity, serverCode: session?.serverCode || config.serverCode || '—', serverName: session?.serverName || config.serverName }); await interaction.reply({ content: '🔴 Server-full announcement sent.', ephemeral: true }); return;
    }
    case 'server-status':
    case 'player-count': await interaction.reply({ content: session ? `🟢 **${session.serverName || config.serverName}** — **${session.currentPlayers}/${session.capacity}** players.` : '⚫ No active roleplay session.', ephemeral: true }); return;
    case 'staff-status':
    case 'staff-roster': {
      const staff = await prisma.member.findMany({ where: { guildId: interaction.guildId, staff: true, onDuty: true }, take: 25 }); await interaction.reply({ content: staff.length ? `👮 Staff on duty: ${staff.map((m) => `<@${m.discordId}>`).join(', ')}` : '⚠️ No staff are currently marked on duty.', ephemeral: true }); return;
    }
    case 'staff-check': {
      const count = await prisma.member.count({ where: { guildId: interaction.guildId, staff: true, onDuty: true } }); await interaction.reply({ content: count ? `✅ Staff presence detected: **${count}** on duty.` : '⚠️ No staff are currently on duty.', ephemeral: true }); return;
    }
    case 'on-duty':
    case 'off-duty': {
      ...
      await writeAudit(interaction.guildId, interaction.user.id, interaction.commandName === 'on-duty' ? 'DUTY_ON' : 'DUTY_OFF', member.id, department ? { department } : {}); await interaction.reply({ content: interaction.commandName === 'on-duty' ? `🟢 You are now on duty${department ? ` for **${department}**` : ''}.` : '⚫ You are now off duty.', ephemeral: true }); return;
    }
    case 'duty-status': {
      const member = await prisma.member.findUnique({ where: { guildId_discordId: { guildId: interaction.guildId, discordId: interaction.user.id } } }); await interaction.reply({ content: member?.onDuty ? '🟢 You are on duty.' : '⚫ You are off duty.', ephemeral: true }); return;
    }
    case 'department-list': {
      const departments = await prisma.department.findMany({ where: { guildId: interaction.guildId, enabled: true }, orderBy: { name: 'asc' } }); await interaction.reply({ content: departments.length ? `👮 **Departments**\n${departments.map((d) => `• ${d.name}`).join('\n')}` : 'No departments configured yet.', ephemeral: true }); return;
    }
    case 'department-create': {
      const name = interaction.options.getString('name', true).trim(); const existing = await prisma.department.findUnique({ where: { guildId_name: { guildId: interaction.guildId, name } } }); if (existing) { await interaction.reply({ content: 'That department already exists.', ephemeral: true }); return; } await prisma.department.create({ data: { guildId: interaction.guildId, name } }); await interaction.reply({ content: `✅ Department created: **${name}**.`, ephemeral: true }); return;
    }
    case 'department-info':
    case 'department-roster':
    case 'department-ranks': {
      const name = interaction.options.getString('department', true); const department = await prisma.department.findUnique({ where: { guildId_name: { guildId: interaction.guildId, name } }, include: { ranks: true } }); await interaction.reply({ content: department ? `👮 **${department.name}**\n${department.description || 'No description configured.'}\nRanks: ${department.ranks.length ? department.ranks.map((r) => r.name).join(', ') : 'None'}` : 'Department not found.', ephemeral: true }); return;
    }
    case 'department-delete': {
      const name = interaction.options.getString('department', true); const department = await prisma.department.findUnique({ where: { guildId_name: { guildId: interaction.guildId, name } } }); if (!department) { await interaction.reply({ content: 'Department not found.', ephemeral: true }); return; } await prisma.department.delete({ where: { id: department.id } }); await interaction.reply({ content: `🗑️ Deleted department **${name}**.`, ephemeral: true }); return;
    }
    case 'ingame-ban':
    case 'ingame-kick':
    case 'ingame-warning': {
      const action = interaction.commandName.replace('ingame-', '').toUpperCase(); const player = interaction.options.getString('player', true); const reason = interaction.options.getString('reason', true); const duration = interaction.options.getInteger('duration'); const record = await prisma.record.create({ data: { guildId: interaction.guildId, scope: 'ingame', action, subjectId: player, reason, durationSeconds: duration ? duration * 60 : null, issuedBy: interaction.user.id, sessionId: session?.id } }); await writeAudit(interaction.guildId, interaction.user.id, `INGAME_${action}`, record.id); await interaction.reply({ content: `📋 In-game **${action.toLowerCase()}** record created. Case: \`${record.id}\`.`, ephemeral: true }); return;
    }
    case 'discord-ban':
    case 'discord-kick': {
      const target = interaction.options.getUser('user', true); const reason = interaction.options.getString('reason', true); const duration = interaction.options.getInteger('duration'); if (interaction.commandName === 'discord-ban') await interaction.guild.members.ban(target, { reason }); else { const member = await interaction.guild.members.fetch(target.id).catch(() => null); if (member) await member.kick(reason); } const action = interaction.commandName.replace('discord-', '').toUpperCase(); const record = await prisma.record.create({ data: { guildId: interaction.guildId, scope: 'discord', action, subjectId: target.id, subjectName: target.username, reason, durationSeconds: duration ? duration * 60 : null, issuedBy: interaction.user.id, sessionId: session?.id } }); await writeAudit(interaction.guildId, interaction.user.id, `DISCORD_${action}`, record.id); await interaction.reply({ content: `🟦 Discord **${action.toLowerCase()}** completed and recorded as case \`${record.id}\`.`, ephemeral: true }); return;
    }
    case 'discord-record':
    case 'ingame-record': {
      const caseId = interaction.options.getString('case-id', true); const record = await prisma.record.findFirst({ where: { id: caseId, guildId: interaction.guildId } }); await interaction.reply({ content: record ? `📋 **Case ${record.id}**\nScope: ${record.scope}\nAction: ${record.action}\nSubject: ${record.subjectName || record.subjectId}\nReason: ${record.reason || '—'}` : 'Record not found.', ephemeral: true }); return;
    }
    case 'ingame-search':
    case 'ingame-records': {
      const query = interaction.options.getString(interaction.commandName === 'ingame-search' ? 'query' : 'player') || ''; const records = await prisma.record.findMany({ where: { guildId: interaction.guildId, scope: 'ingame', OR: [{ subjectId: { contains: query, mode: 'insensitive' } }, { id: { contains: query } }] }, orderBy: { createdAt: 'desc' }, take: 10 }); await interaction.reply({ content: records.length ? records.map((r) => `\`${r.id}\` • ${r.action} • ${r.subjectId} • ${r.reason || '—'}`).join('\n') : 'No matching in-game records found.', ephemeral: true }); return;
    }
    case 'discord-records': {
      const target = interaction.options.getUser('user'); const records = await prisma.record.findMany({ where: { guildId: interaction.guildId, scope: 'discord', ...(target ? { subjectId: target.id } : {}) }, orderBy: { createdAt: 'desc' }, take: 10 }); await interaction.reply({ content: records.length ? records.map((r) => `\`${r.id}\` • ${r.action} • ${r.subjectName || r.subjectId} • ${r.reason || '—'}`).join('\n') : 'No Discord records found.', ephemeral: true }); return;
    }
    case 'commands': {
      const categories = new Map<string, string[]>(); for (const command of COMMANDS) (categories.get(command.category) || (categories.set(command.category, []), categories.get(command.category)!)).push(`/${command.name}`); const description = Array.from(categories.entries()).map(([category, names]) => `**${category}**\n${names.join(' • ')}`).join('\n\n'); await interaction.reply({ embeds: [new EmbedBuilder().setTitle('🧰 DRP Command Center').setDescription(description.slice(0, 3900)).setColor('#5865F2')], ephemeral: true }); return;
    }
    case 'command':
    case 'help': {
      const name = interaction.commandName === 'help' ? interaction.options.getString('command') : interaction.options.getString('name'); const target = COMMANDS.find((c) => c.name === name); if (!target) { await interaction.reply({ content: 'Command not found. Use `/commands` to browse the registry.', ephemeral: true }); return; } await interaction.reply({ embeds: [new EmbedBuilder().setTitle(`/${target.name}`).setDescription(target.description).addFields({ name: 'Usage', value: `\`${target.usage}\`` }, { name: 'Permissions', value: target.permissions.length ? target.permissions.join(', ') : 'Everyone' }, { name: 'Cooldown', value: `${target.cooldownSeconds}s` }).setColor('#5865F2')], ephemeral: true }); return;
    }
    ...
    case 'health':
    case 'bot-status': { const hb = await prisma.botHeartbeat.findUnique({ where: { guildId: interaction.guildId } }); await interaction.reply({ content: `🤖 ${hb?.status === 'online' ? 'Online' : 'Offline'} • DB **connected** • Gateway **${client.ws.ping}ms** • Heartbeat **${hb ? `<t:${Math.floor(hb.lastSeen.getTime() / 1000)}:R>` : 'not recorded'}** • Maintenance **${hb?.maintenance ? 'ON' : 'OFF'}**`, ephemeral: true }); return; }
    default: await interaction.reply({ content: `**/${definition.name}** is registered. Its dashboard-backed workflow can be configured from DRP Control.`, ephemeral: true });
  }
}

client.once('ready', async () => {
  console.log(`[DRP] Logged in as ${client.user?.tag}`);
  try { await registerCommands(); console.log(`[DRP] Registered ${COMMANDS.length} slash commands`); } catch (error) { console.error('[DRP] Command registration failed:', error); }
  const firstGuild = client.guilds.cache.first();
  if (firstGuild) { const { config } = await getConfig(firstGuild.id); await syncPresence(config).catch((error) => console.error('[DRP] Presence sync failed:', error)); }
  await heartbeat().catch((error) => console.error('[DRP] Heartbeat failed:', error));
});

client.on('interactionCreate', async (interaction) => {
  if (!interaction.isChatInputCommand()) return;
  try { await handleCommand(interaction); } catch (error) { console.error(`[DRP] /${interaction.commandName} failed:`, error); if (interaction.replied || interaction.deferred) await interaction.followUp({ content: 'An internal error occurred while running that command.', ephemeral: true }).catch(() => undefined); else await interaction.reply({ content: 'An internal error occurred while running that command.', ephemeral: true }).catch(() => undefined); }
});
